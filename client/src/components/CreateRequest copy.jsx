...
const CreateRequest = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(() => fetchBarang(), 300); // Debouncing
  ...
  const addToCart = () => {
    ...
    if (isNaN(inputQty) || inputQty <= 0) return alert("Jumlah (Qty) harus valid!");
    if (inputQty > selectedBarang.stok) return alert(`Stok tidak mencukupi! Sisa: ${selectedBarang.stok}`);
  ...
  const handleSimpanTransaksi = async () => {
    if (!user?.id_user) return alert("Sesi user tidak ditemukan. Silahkan login ulang.");
